...
//$addres = '192.168.1.77';
$master  = WebSocket('192.168.1.77',57362);
...
function WebSocket($address,$port){
 ...
 // socket_bind($master, $address)        or die("socket_bind() failed");

 for ( $port = 1234; $port < 65536; $port++ )
{
    echo "\n port= $port \n";
    $result = socket_bind($master, $address) ;
    //or die("\n Could not bind to socket\n");
    if ( $result )
    {
        echo "bind succeeded, port=$port\n";
        break;
    } else {
        echo "Binding to port $port failed: ";
        print socket_strerror(socket_last_error($master))."\n";
    }
}

   echo "socket binding complete \n";
